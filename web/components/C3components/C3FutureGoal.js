...
const totalScore = (array) => array.reduce((previousValue, currentValue) => previousValue + currentValue.expected_score, 0);
const totalMaxScore = (array) => array.reduce((previousValue, currentValue) => previousValue + currentValue.max_score, 0);

const C3FutureGoal = ({target_details, targetId}) => {
  ...
  if (Object.keys(targetDetailsCategories).length){
    return (
      <div className="w-full px-8 flex flex-col items-center py-4 bg-white mb-2">
        ...
        <Link href={`/naiteishaplan/edit-target/` + targetId } className="flex">
        ...
        <div className="flex w-full flex-col gap-3 mt-2">
          { Object.keys(targetDetailsCategories).map(key => {
                        ...
                        { element?.test_content?.score_eachs.map((part) => {
                          return (
                            <div key={part?.part_name}>
                              <span>・{ part?.part_name }({ part?.max_score })：{ part?.expected_score }</span><br/>
                            </div>
                          );
                        })}
                        </div>
            ...
            } else if (targetDetailsCategories[key][0].type === 1) {
              return (
    ...
    return (
      <div className="w-full px-8 flex flex-col items-center py-4 bg-white mb-2 min-h-[500px]">
        <div className='text-xl'>目標なし...</div>
      </div>
    );
  }
};
