...
import EditAchievementShow from "~/components/C3components/Target-Achievement/EditAchievement";
...
const EditAchievement = () => {
  ...
  const handleSubmit = async () => {
    if (state?.canSubmit) {
      ...
      const json = await ky
        .post(`/api/target/update-result`, { json: state.milestone.target })
        .json();
      if (json.success) {
        hide();
        openNotification({
    ...
    <HomeLayout userInfo={userInfo} active={"学習目標"}>
      <div className="wrapper overflow-y-auto w-full">
        <div className="bg-white my-3 rounded-[20px] pl-8 pt-8 pb-8">
          ...
          <EditAchievementShow target={state?.milestone?.target} state={state} dispatch={dispatch} />
